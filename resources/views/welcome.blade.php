        ...
        <div class="row align-items-center g-5">
            <div class="col-lg-6">
                ...
                <p class="lead text-secondary mb-4">
                    Discover refined essentials, verified merchants, secure checkout, and fast delivery in one polished shopping experience.
                </p>
                <div class="d-flex flex-wrap gap-3 mb-4">
                    ...
                    @guest
                        <a href="{{ route('register') }}" class="btn btn-outline-dark btn-lg btn-premium px-4">Create Account</a>
                    @endguest
                </div>
                <div class="row g-3">
                    <div class="col-4">
            ...
            <div class="col-lg-6">
                <div class="surface overflow-hidden">
                    <img
                        ...
                        class="w-100"
                        style="height:min(560px, 70vh); object-fit:cover;"
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-between gap-4 mb-4">
            ...
            <div class="d-flex flex-wrap align-items-center gap-2">
                @foreach ($featuredCategories as $category)
                    <a href="{{ route('category.products', $category->id) }}" class="btn btn-outline-dark btn-premium btn-sm px-3">
                        {{ $category->name }}
                    </a>
                @endforeach
                        ...
                        <article class="card product-card h-100">
                            <div class="product-media">
                                ...
                                @auth
                                    @if (auth()->user()->isCustomer())
                                        <form method="POST" action="{{ route('wishlist.toggle', $product) }}" class="floating-action" data-ajax-wishlist="true" data-product-id="{{ $product->id }}">
                                            @csrf
                                            <button type="submit" class="btn {{ $isWishlisted ? 'btn-dark' : 'btn-light' }} btn-icon shadow-sm" aria-label="Wishlist">
                            ...
                            <div class="card-body d-flex flex-column p-3">
                                <div class="d-flex justify-content-between align-items-center gap-2 mb-2">
                                    <span class="small text-muted fw-semibold text-truncate">{{ $product->category->name ?? 'Collection' }}</span>
                                ...
                                <h6 class="fw-bold mb-2">
                                    <a href="{{ route('product.view', $product) }}" class="text-dark text-decoration-none">{{ $product->name }}</a>
                                </h6>
                                <small class="text-muted mb-3"><i class="bi bi-shop me-1"></i>{{ $product->user->name ?? 'Merchant' }}</small>
