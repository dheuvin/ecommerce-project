                ...
                <div class="position-sticky" style="top: 104px;">
                    <div class="surface overflow-hidden">
                        @if ($product->primary_image_path)
                            <img src="{{ asset('storage/' . $product->primary_image_path) }}" class="w-100" style="aspect-ratio:1 / 1; object-fit:contain; background:#f7f8fb;" alt="{{ $product->name }}">
                        @else
                            <div class="d-flex flex-column align-items-center justify-content-center text-muted" style="aspect-ratio:1 / 1; background:#f7f8fb;">
                    ...
                    @if ($product->images->count())
                        <div class="d-flex gap-2 mt-3 flex-wrap">
                            @foreach ($product->images as $image)
                                <img src="{{ asset('storage/' . $image->image) }}" width="84" height="84" class="rounded-4 border bg-light" style="object-fit:cover;" alt="{{ $product->name }} gallery image">
                            @endforeach
                        </div>
                    @endif
                </div>
            ...
            <div class="col-lg-6">
                ...
                <div class="d-flex flex-wrap align-items-center gap-3 mb-4">
                    <span class="rating-stars">
                        @for ($i = 1; $i <= 5; $i++)
                            <i class="bi {{ $averageRating && $i <= round($averageRating) ? 'bi-star-fill' : 'bi-star' }}"></i>
                        @endfor
                    </span>
                ...
                <div class="soft-surface p-4 mb-4">
                    <div class="d-flex justify-content-between align-items-start gap-3">
                        ...
                        <span class="badge rounded-pill px-3 py-2 {{ $product->stock > 0 ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }}">
                ...
                <div class="d-flex gap-2 flex-wrap mb-5">
                    @auth
                        ...
                        @endif
                    @else
                        <a href="{{ route('login') }}" class="btn btn-dark btn-premium px-4">Sign In to Buy</a>
                    @endauth

                    @auth
                        @if (auth()->user()->isSeller() && auth()->id() === $product->user_id)
                            <a href="{{ route('products.edit', $product) }}" class="btn btn-outline-dark btn-premium px-4">Edit Product</a>
                        @endif
                    @endauth

                    <a href="{{ url()->previous() }}" class="btn btn-light border btn-premium px-4">Back</a>
                </div>
                ...
                <div class="surface p-4">
                    <button class="btn btn-link text-dark fw-bold fs-5 p-0 text-decoration-none" type="button" data-bs-toggle="collapse" data-bs-target="#reviewBox" aria-expanded="true">
                    ...
                    <div class="collapse show mt-4" id="reviewBox">
                        ...
                        @else
                            <a href="{{ route('login') }}" class="btn btn-dark btn-premium btn-sm mb-3">Login to write review</a>
                        @endauth
                            ...
                            <article class="card product-card h-100">
                                <div class="product-media">
                                    @if ($item->primary_image_path)
                                        <img src="{{ asset('storage/' . $item->primary_image_path) }}" alt="{{ $item->name }}">
                                    @else
